...
faceMesh.onResults((results) => {
  ...
  if (results.multiFaceLandmarks && results.multiFaceLandmarks[0]) {
    ...
    if (earringImg && earringImg.complete) {
      ...
      if (ratio > 0.2) { canvasCtx.save(); canvasCtx.translate(leftEar.x, leftEar.y + (ew * 0.15)); canvasCtx.rotate(physics.earringAngle); canvasCtx.drawImage(earringImg, -ew/2, 0, ew, eh); canvasCtx.restore(); }
      if (ratio < 0.8) { canvasCtx.save(); canvasCtx.translate(rightEar.x, rightEar.y + (ew * 0.15)); canvasCtx.rotate(physics.earringAngle); canvasCtx.drawImage(earringImg, -ew/2, 0, ew, eh); canvasCtx.restore(); }
    }
